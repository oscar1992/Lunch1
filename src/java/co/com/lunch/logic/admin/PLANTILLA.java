...
import co.com.lunch.persistencia.admin.NOMBREENTIDAD;
...
public class NOMBRECLASE {
     ...
     * @return 
     */
    public NOMBREENTIDAD ingresaNOMBREMETODO(NOMBREENTIDAD info){
        NOMBREENTIDAD infoRetorno=null;
        try{
            if(initOperation()){
                info.setId(maxId());
                sesion.save(info);
     ...
     * @return 
     */
    public NOMBREENTIDAD actualizaNOMBREMETODO(NOMBREENTIDAD info){
        NOMBREENTIDAD infoRetorno=null;
        try{
            if(initOperation()){
                sesion.update(info);
     ...
     * @return 
     */
    public ArrayList<NOMBREENTIDAD> listaNOMBREMETODO(){
        ArrayList<NOMBREENTIDAD>lista=new ArrayList<>();
        try{
            if(initOperation()){
                Criteria criteria=sesion.createCriteria(NOMBREENTIDAD.class);
                lista=(ArrayList<NOMBREENTIDAD>)criteria.list();
            }else{
                System.out.println("ERROR de validación al conectar");
            }
        }catch(Exception e){
    ...
    private Integer maxId(){
        ...
        try{
            if(initOperation()){
                Query query=sesion.createQuery("SELECT MAX(id) FROM NOMBREENTIDAD");
