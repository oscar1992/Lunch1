...
import co.com.lunch.persistencia.admin.TIPOENTIDAD;
...
public class NOMBRECLASE {
     ...
     * @return 
     */
    public TIPOENTIDAD ingresaNOMBREMETODO(TIPOENTIDAD info){
        TIPOENTIDAD infoRetorno=null;
        try{
            if(initOperation()){
                info.setId(maxId());
                sesion.save(info);
     ...
     * @return 
     */
    public TIPOENTIDAD actualizaNOMBREMETODO(TIPOENTIDAD info){
        TIPOENTIDAD infoRetorno=null;
        try{
            if(initOperation()){
                sesion.update(info);
     ...
     * @return 
     */
    public ArrayList<TIPOENTIDAD> listaNOMBREMETODO(){
        ArrayList<TIPOENTIDAD>lista=new ArrayList<>();
        try{
            if(initOperation()){
                Criteria criteria=sesion.createCriteria(TIPOENTIDAD.class);
                lista=(ArrayList<TIPOENTIDAD>)criteria.list();
            }else{
                System.out.println("ERROR de validación al conectar");
            }
        }catch(Exception e){
    ...
    private Integer maxId(){
        ...
        try{
            if(initOperation()){
                Query query=sesion.createQuery("SELECT MAX(id) FROM TIPOENTIDAD");
